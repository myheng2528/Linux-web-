...
//
/*
正则表达式 用于解析请求行的三个部分：方法、路径和 HTTP 版本。
如果请求行符合预期格式，解析成功并返回 true；否则返回 false。
成员变量 method_、path_ 和 version_ 用来存储解析后的值。
*/
...
bool HttpRequest::IsKeepAlive() const {
    ...
    if (it != header_.end() && it->second == "keep-alive") {
        return version_ == "1.1"; // 检查 HTTP 版本 
    }
    return false; // 要么没有 "Connection" 要么不是 keep-alive
}
