...
void BlockQueue<T>::push_back(const T& item) {
    ...
    unique_lock<mutex> locker(mtx_);       // 队列满了，需要等待
    condProducer_.wait(locker,[this](){return deq_.size()<capacity_|| isClose_;});     // 暂停生产，等待消费者唤醒生产条件变量
    if (isClose_)
    {
        return;
    }
...
bool BlockQueue<T>::pop(T &item, int timeout) {
    ...
    */
   //只要满足条件非空或关闭就结束阻塞
    auto isTimeOut = condConsumer_.wait_for(locker, std::chrono::seconds(timeout), [this]() { 
        return isClose_ || !deq_.empty();
    });
    //如果超时或关闭就返回
    if (isClose_||isTimeOut==std::cv_status::timeout)
    {
        return false;
    }
